...
export const contact = async (context: RouterContext) => {
  ...
  if(!contact.contactName){
    ...
    context.response.body=JSON.stringify({"error" : "Un des champs est manquant"});
  }
...
export const sendEmail = async (context: RouterContext<{ id_us: string}>) => {
  ...
  if(!emailValidation.test(idUs)){
    context.response.status = 400;
    context.response.body = JSON.stringify({success: false, error: "l'address email n'est pas vailde"});
  }
  let contacts: any= []; 
  ...
  await axiod
    ...
    .then((response) =>  context.response.body = JSON.stringify({success : true, response }))
    .catch((error) =>  context.response.body = JSON.stringify({success: false, error }))
}
contactController
